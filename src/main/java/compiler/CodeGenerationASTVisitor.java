...
public class CodeGenerationASTVisitor extends BaseASTVisitor<String, VoidException> {
    ...
    public String visitNode(ClassNode n) {
        ...
        for (var method : dispatchTable) {
            dispatchTableCode = nlJoin(
                    dispatchTableCode,
                    // Memorize method address in the dispatch table (which is stored in the Heap)
                    "push " + method,
                    "lhp",
                    "sw",
                    // Increment HP
    ...
    public String visitNode(NewNode n) {
        ...
        for (int i = 0; i < n.argList.size(); i++) {
            argToHeap = nlJoin(
                    argToHeap,
                    // Put argument value in the Heap
                    "lhp",
                    "sw",
                    // Increment HP
                    "lhp",
        ...
        return nlJoin(
                argValueCode,
                argToHeap,
                // Get value from MEMSIZE - offset
                "push " + (MEMSIZE + n.entry.offset),
                "lw",
                // Put it in the Heap
                "lhp",
                "sw",
                // Push the HP before incrementing it
                "lhp",
                // Increment the HP
                "lhp",
                "push 1",
